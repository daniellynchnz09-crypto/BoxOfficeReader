...
6. A line graph on each movie detail page visualizing that single film's week-by-week trajectory, per the user: "based on the graph on sheet 3" — note the "box office test" spreadsheet is only confirmed to have Sheet1 and Sheet2 as of this session; verify whether a Sheet3 actually exists (or whether the user means the Slides deck chart) before building this.
