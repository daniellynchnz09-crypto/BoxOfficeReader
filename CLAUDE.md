...
- Movie detail page load times: added `generateStaticParams()` (returning `encodeURIComponent`-ed names — plain names broke the Windows build because `:` isn't a legal path character there) and `export const revalidate = 21600` to `app/movie/[name]/page.tsx`, and raised `getFilmDetail()`'s fetch `revalidate` from 300s to `21600` (6h) in `lib/n8n.ts`. Build output now shows all 25 films prerendered as `●` SSG pages with a 6h revalidate window instead of `ƒ` fully-dynamic.
